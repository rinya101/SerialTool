...
class SerialTool(ctk.CTk):
    ...
    # 定义统一的样式颜色（便于维护）
    COLOR_SYSTEM = "#e74c3c"      # 系统提示色（红）
    COLOR_RECEIVE = "#3498db"     # 接收数据色（蓝）
    COLOR_SEND = "#2ecc71"        # 发送数据色（绿）
    COLOR_ERROR = "#c0392b"       # 错误提示色（深红）
    COLOR_RECEIVE_ERROR = "#ff0000"  # 接收错误提示色（深蓝）
    # 消息标签颜色字典
    message_label_color_dict = {
        "system": COLOR_SYSTEM,
        "receive": COLOR_RECEIVE,
        "send": COLOR_SEND,
        "error": COLOR_ERROR,
        "receive_error": COLOR_RECEIVE_ERROR
    }
    ...
    def __init__(self, fg_color=None, **kwargs):
        ...
        self._append_receive_text("测试 - system\n", tag = "system")
        self._append_receive_text("测试 - receive\n", tag = "receive")
        self._append_receive_text("测试 - send\n", tag = "send")
    ...
    def _configure_text_tags(self):
        ...
        for tag in self.message_label_color_dict:
            self.text_widget.tag_configure(tag, foreground = self.message_label_color_dict[tag])
                ...
                if self.serial_port and self.serial_port.in_waiting > 0:
                    ...
                    if data:
                        ...
                        if data != '\r' or data != '\n':
                            ...
                            self._append_receive_text(buffer, "receive")
                time.sleep(0.01)
            except Exception as e:
                if self.is_serial_open:
    ...
    def _system_message(self, message, tag):
        ...
        color_map = {
            ...
            "warning": "#FF9900",
            "info": self.COLOR_RECEIVE,
            "success": self.COLOR_SEND,
            "system": self.COLOR_SYSTEM
        }
        self.system_tip_label.configure(
            text_color=color_map.get(tag, self.COLOR_SYSTEM),
            text=f"{message}",
        )
